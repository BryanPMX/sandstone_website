...
const SANTA_TERESA_CENTER: [number, number] = [31.8559, -106.6392];
const SANTA_TERESA_ZOOM = 12;
const PAGE_SIZE = 6;
...
function normalizeListingsResponse(data: unknown): PropertyCard[] {
  if (Array.isArray(data)) return data as PropertyCard[];
  ...
  if (data && typeof data === "object") {
    ...
    if (Array.isArray(response.properties)) {
      return response.properties as PropertyCard[];
    }

    if (Array.isArray(response.listings)) {
      return response.listings as PropertyCard[];
    }

    if (Array.isArray(response.D?.Results)) {
      return response.D.Results as PropertyCard[];
    }
  }
...
export function SantaTeresaListings() {
  ...
  useEffect(() => {
    ...
    fetch("/api/listings/santa-teresa-nm")
      .then((r) => r.json())
      .then((data: unknown) => setAllListings(normalizeListingsResponse(data)))
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,1fr)]">
        <ListingsMapPanel
          properties={filtered}
          initialCenter={SANTA_TERESA_CENTER}
          initialZoom={SANTA_TERESA_ZOOM}
          mapContextQuery={{ from: "santa-teresa-nm" }}
        />
              ...
              {Array.from({ length: totalPages }, (_, k) => k + 1)
                ...
                .map((item, idx) =>
                  item === "..." ? (
                    <span
                      key={`ellipsis-${idx}`}
                      className="px-1 text-sm text-[var(--sandstone-charcoal)]/40"
                    >
                      ...
                    </span>
                  ) : (
